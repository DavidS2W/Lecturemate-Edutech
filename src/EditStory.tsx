...
function EditStoryPage(){
    ...
    useEffect(()=>{
        async function grabData(){
            ...
            if (courseData){
                let stories = courseData.stories;
                let story = stories.find((e:any)=>{return e.id === storyID});
                console.log(story);
                setTitle(story.title);
                setDesc(story.description);
                setEpisodeList(story.episodes);
    ...
    async function uploadCourse(){
        if(imgFile !== null){
           let imageURL = courseID && storyID && await uploadImg(courseID, storyID);
        ...
        let newList = initialStoryList.filter((e:any)=>{return e.id !== storyID});
        newList.push({title: title, description: desc, image: imgURL, episodes: episodeList, id: storyID});
        courseID && await updateDoc(doc(db, "course_db", courseID), {stories: newList});
    ...
    return(
        <div className="createpage-body">
            <NavBar/>
            <div className="createpage-header">Editing this story: {title}</div>
            <div className="createpage-entry-container">
                ...
                <div className="createpage-entry-field-final">
                    <button className="createpage-entry-button" disabled={buttonState} onClick={async()=>{await uploadCourse()}}>Update Story</button>
                </div>
            </div>
        </div>
    )
