...
function CreateEpisodePage(){
    ...
    async function uploadCourse(){
        const episodeID = title.toLowerCase().replace(/\s+/g, '_');
        const imageURL = courseID && storyID && await uploadImg(courseID, storyID, episodeID);
        ...
        initialEpisodeList.push({title: title, description: desc, image: imageURL, questions: [], id: storyID});
        courseID && await updateDoc(doc(db, "course_db", courseID), {episodes: initialStoryList});
        navigate(`/admin/create/${courseID}/${storyID}/${episodeID}`);
    }
    ...
    return(
        <div className="createpage-body">
            <NavBar/>
            <div className="createpage-header">Create a new episode</div>
            <div className="createpage-entry-container">
                ...
                <div className="createpage-entry-field-final">
                    <button className="createpage-entry-button" disabled={buttonState} onClick={async()=>{await uploadCourse()}}>Create Episode</button>
                </div>
            </div>
        </div>
    )
